...
SEC_FILING_LIMITATIONS_NOTE = (
    ...
    "- 10-K/10-Q는 primaryDocument HTML에서 텍스트를 추출해 전문을 캡처한다(태그만 제거한 단순 "
    "변환이라 표 구조가 완벽히 보존되진 않는다). 8-K는 Exhibit 99.x 중 컨퍼런스콜 "
    "녹취록으로 보이는 것이 있으면 그 원문을, 없어도 실적발표(항목 2.02) 관련 8-K라면 실적 보도자료 "
    "exhibit 원문을 캡처한다 - 그 외(합병/임원변경 등 실적과 무관한 8-K)는 메타데이터와 원문 링크만 "
    "캡처한다.\n"
...
_BODY_NOTE_BY_CAPTURE_KIND: dict[str, str] = {
    "transcript": "(8-K 첨부 컨퍼런스콜 녹취록 원문)",
    "press_release": "(8-K 첨부 실적발표 보도자료 원문)",
...
def render_sec_filing_body(
    ...
    body_note = _BODY_NOTE_BY_CAPTURE_KIND.get(capture_kind) if full_text else None
