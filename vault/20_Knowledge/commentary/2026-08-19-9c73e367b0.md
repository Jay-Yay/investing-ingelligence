...
title: '앤스로픽, Claude로 단백질 설계, 분석화학 자동화 성과 공개 : 앤스로픽은 Claude가 신약개발 초기'
description: '텔레그램 채널 삼성 글로벌AI/SW 이영진의 2026-08-19 메시지. 앤스로픽, Claude로 단백질 설계, 분석화학 자동화 성과 공개 : 앤스로픽은 Claude가
  신약개발 초기 단계의 핵심 업무인 단백질 결합체 설계와 분석화학 워크플로를 상당 부분 자동화할 수 있음'
resource: https://t.me/Samsung_Global_AI_SW/4305
tags:
- telegram
- ch-telegram-samsung-global-ai-sw
- ko
timestamp: '2026-08-19T08:04:22Z'
status: stable
language: ko
capture: full
period:
...
provenance:
  system: telegram
  content_hash: 9c73e367b0acb675
  source_path: vault/10_Sources/Telegram/Samsung_Global_AI_SW/2026/2026-08-19-9c73e367b0acb675.md
---

# 요약

텔레그램 채널 삼성 글로벌AI/SW 이영진의 2026-08-19 메시지. 앤스로픽, Claude로 단백질 설계, 분석화학 자동화 성과 공개 : 앤스로픽은 Claude가 신약개발 초기 단계의 핵심 업무인 단백질 결합체 설계와 분석화학 워크플로를 상당 부분 자동화할 수 있음

# 원문

## 원문
Samsung_Global_AI_SW (@Samsung_Global_AI_SW) — 2026-08-19T08:04:22+00:00

앤스로픽, Claude로 단백질 설계, 분석화학 자동화 성과 공개

: 앤스로픽은 Claude가 신약개발 초기 단계의 핵심 업무인 단백질 결합체 설계와 분석화학 워크플로를 상당 부분 자동화할 수 있음을 입증한 실험 결과 공개

: 단백질 설계에서는 Claude Mythos Preview, Opus 4.8이 15개 표적 중 14개에서 결합체 설계에 성공. 전체 1,320개 설계 중 354개가 실제 결합체로 확인

: 다중 표적 동시 설계 기준 적중률은 Mythos Preview 26.7%, Opus 4.8 22.6%, 표적별 개별 설계 시 Mythos Preview는 35.1%까지 상승. 현재 일반적인 단백질 설계 캠페인 적중률인 10~15%를 크게 상회

: 최소 6개 표적에서 고친화도 결합체를 확보했고, 최소 4개 표적에서는 기존 최고 공개 결과와 동등하거나 더 높은 친화도 기록. RBX1에서는 Mythos Preview가 40% 적중률을 기록해 기존 대회 참가자 평균 3.7%를 크게 상회

: 난도가 높은 TNFα에서는 Opus 4.8이 사람·원숭이·생쥐에 모두 결합하는 종간 교차반응 결합체까지 설계. 반면 MBP에서는 90개 설계 모두 결합 확인에 실패하는 등 표적별 성능 편차 존재

: 설계 과정은 초기 프롬프트 이후 인간의 과학적 개입 없이 진행. Claude가 공개 단백질 설계·폴딩 모델과 GPU를 직접 조율해 표적 부위 선정→후보 구조·서열 생성→인실리코 최적화→후보 스크리닝까지 자율 수행

: 분석화학에서는 일반 이용 가능한 Claude Opus 5에 NMR·LC-MS 원시 파일과 두 문장짜리 프롬프트만 제공. 별도 전문 소프트웨어나 작업자 없이 각각 23분·19분 만에 분석 완료

: Claude의 결과는 외부 분석기관과 거의 일치. NMR 피크별 수소 개수 오차는 0.08 1H 이내였고, LC-MS 순도는 Claude 96.4% vs 분석기관 96.33%

: Claude는 제조사 비공개 포맷의 LC-MS 파일 구조까지 스스로 해석하고 2,664개 스캔의 무결성을 검증했으며, NMR에서는 실제 분석기관과 동일한 중수 후속 검증 실험을 독립적으로 제안하는 등 과학적 판단도 확인

: 이번 결과를 신약개발 전 과정의 에이전트화로 가는 기반 기술로 평가. 다만 단백질 설계 등 고위험 생물학 역량은 이중용도 위험 때문에 최상위 모델에서 일반 공개하지 않고, 과학자 대상 별도 접근 프로그램을 준비 중
...
Read more
An unreleased research version of Claude has made strides on a problem related to the Riemann hypothesis. It improved a longstanding lower bound for the fraction of zeros of the Riemann zeta function that satisfy the hypoth

[...이하 생략, 원문 총 20,050자 중 20,000자까지만 캡처됨. 전체 원문은 출처 링크 참고...]

## 출처
- [원문](https://t.me/Samsung_Global_AI_SW/4305)
- [기사 원문](https://www.anthropic.com/research/Claude-accelerates-protein-design)
...
- 수집 시스템: `telegram`
- 원본 파일: `vault/10_Sources/Telegram/Samsung_Global_AI_SW/2026/2026-08-19-9c73e367b0acb675.md`
- 내용 해시: `9c73e367b0acb675…`
- 원문: <https://t.me/Samsung_Global_AI_SW/4305>
